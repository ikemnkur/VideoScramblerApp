...
export default function PhotoScramblerPro() {
    ...
    const [noiseIntensity, setNoiseIntensity] = useState(32); // Noise intensity for obscuring the image (0-127)
 ...
 ========================= */
    ...
    // ADD NOISE TO IMAGE
    // =============================
    const applyNoiseToImageFile = async (file) => {
        if (noiseIntensity <= 0) {
            console.log("No noise to apply (intensity = 0)");
            return file;
        }
        ...
        try {
            ...
            if (tile > 0) {
                ...
                console.log("Noise applied to image before scrambling", { intensity, tile, seed: noiseSeed });
            }

            // Convert canvas back to file
            return new Promise((resolve) => {
                canvas.toBlob((blob) => {
                    const noisyFile = new File([blob], file.name, { type: file.type });
                    resolve(noisyFile);
                }, file.type);
            });
        } catch (err) {
            console.error("Error applying noise:", err);
            return file; // Return original file if noise application fails
        }
    };
    ...
    const scrambleImage = async (file) => {
        ...
        try {
            // Apply noise to image before scrambling if noise intensity > 0
            let fileToScramble = selectedFile;
            if (noiseIntensity > 0) {
                fileToScramble = await applyNoiseToImageFile(selectedFile);
            }

            // Build scramble parameters based on algorithm
            const params = {
                input: fileToScramble.name,
                output: `scrambled_${fileToScramble.name}`,
                seed: seed,
                mode: 'scramble'
            };
            ...
            formData.append('file', fileToScramble);
