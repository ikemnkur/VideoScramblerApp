...
export default function VideoUnscramblerBasic() {
  ...
  const jsonToParams = (obj) => {
    ...
    const n = Number(scrambleData.n), m = Number(scrambleData.m);
    let perm = null;
  ...
  const applyParameters = () => {
    try {
      ...
      const noiseInfo = noise ? ` with noise intensity: ${noise.intensity}` : '';
      ...
      if (storedSrcToDest && storedSrcToDest.length === n * m) {
        ...
        console.log("Current srcToDest: ", srcToDest);
        if (permDestToSrc0.length === n * m) {
          const invPerm = inversePermutation(permDestToSrc0);
          setSrcToDest(invPerm);
          localStorage.setItem('srcToDest', JSON.stringify(invPerm));
          console.log("Calculated and stored srcToDest:", invPerm);
        } else {
  ...
  const drawUnscrambledFrame = useCallback((targetCanvas = null) => {
    ...
    // Step 2: Remove noise if present
    const noiseData = unscrambleParams.noise;
    if (noiseData && noiseData.seed && noiseData.intensity > 0) {
      try {
        ...
        if (tile > 0) {
          // Regenerate same tile offsets used during scrambling
          const tileOffsets = generateNoiseTileOffsets(tile, noiseSeed, intensity);

          // Apply reverse noise (subtract mod 256)
          const cleanImageData = applyNoiseSubMod256(frameImageData, tileOffsets, tile);

          // Put clean data back
          ctx.putImageData(cleanImageData, 0, 0);
        }
      } catch (err) {
        console.error("Error removing noise from frame:", err);
      }
    }

    // Step 3: Add transparent watermark overlay to indicate unscrambled
    ctx.fillStyle = 'rgba(255, 255, 255, 0.1)';
    ctx.font = '15px Arial';
    ctx.fillText(`🔓 Unscrambled by: ${userData.username}`, 10, canvas.height / 2 + 7);

  }, [srcToDest, rectsSrcFromShuffled, rectsDest, unscrambleParams, userData]);
